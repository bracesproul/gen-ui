...
The user input is in response to a 'magic filter' prompt. They expect their natural language description of the filters to be converted into a structured query.`,
  ...
  const ui = await createRunnableUI(config, buttonsDiv);
...
async function generateChartType(
  state: AgentExecutorState,
  config?: RunnableConfig,
): Promise<Partial<AgentExecutorState>> {
  if (!state.ui) {
    throw new Error("UI stream is required to generate the chart type.");
  }
  const prompt = ChatPromptTemplate.fromMessages([
    [
      "system",
...
Generated filters: {selectedFilters}`,
  ...
  if (result.chartType === "bar") {
    // state.ui.append(<LoadingBarChart />);
    state.ui.update(<LoadingBarChart />);
  } else if (result.chartType === "pie") {
    // state.ui.append(<LoadingPieChart />);
    state.ui.update(<LoadingPieChart />);
  }

  return result;
}

async function generateDataDisplayFormat(
  state: AgentExecutorState,
  config?: RunnableConfig,
): Promise<Partial<AgentExecutorState>> {
  const prompt = ChatPromptTemplate.fromMessages([
...
function constructChartProps(
  state: AgentExecutorState,
  config?: RunnableConfig,
): Partial<AgentExecutorState> {
  if (!state.chartType) {
    ...
    throw new Error("UI stream is required to generate the chart props.");
  }
  ...
  if (state.chartType === "bar") {
    state.ui.update(<BarChart {...(props as BarChartProps)} />);
  } else if (state.chartType === "pie") {
    state.ui.update(<PieChart {...(props as PieChartProps)} />);
  }
  state.ui.done();
...
export function graphAgentExecutor() {
  const workflow = new StateGraph<AgentExecutorState>({
    ...
    .addNode("generateFilters", generateFilters) // Generate the filters
    .addNode("generateChartType", generateChartType) // Generate the type of chart to display the data
    .addNode("generateDataDisplayFormat", generateDataDisplayFormat) // Generate the format to display the data
    .addNode("filterData", filterData) // Perform the data filtering
    .addNode("constructChartProps", constructChartProps) // Construct the props for the chart
